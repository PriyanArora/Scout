// Scout grounded tool catalog — single TS source of the 43-tool identity.
//
// This array is the canonical in-code catalog. `agent/src/utils/catalog.ts`
// derives CATALOG_IDS from it; `agent/src/prompts/system-prefix.ts` renders it
// into the shared cacheable system prefix. The human-authored source remains
// `agent/catalog.yaml` (and the SQL seed); a drift test (Wave 2) asserts the
// ids here match those, so the four representations cannot silently diverge.
...
// Render the catalog as one stable, deterministic block for the cacheable prefix.
export function renderCatalogBlock(): string {
  return CATALOG_TOOLS.map((t) => `- ${t.id} (${t.name}) — ${t.whatItDoes}`).join("\n");
}
