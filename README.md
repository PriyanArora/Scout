...
**$0/month infrastructure.** Supabase Free + Vercel Hobby. Claude API tokens are the only marginal cost (~$0.20–$0.50 per run).
...
Each graph node is one Edge Function invocation. The function writes a checkpoint to Postgres, self-chains to the next node, and returns 200 immediately. `pg_cron` recovers any dropped chain. See `docs/ARCHITECTURE.md` for the full design.

## Running a Discovery

### Via the Web UI

1. Deploy Scout (see `docs/RUNBOOK.md`)
...
Full instructions: `docs/RUNBOOK.md`. Quick summary:
