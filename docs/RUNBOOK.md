...
```sql
SELECT id, url, status, next_node, attempts, created_at, lease_until
FROM runs
...
```sql
SELECT id, url, status, next_node, lease_until, updated_at
FROM runs
...
```sql
SELECT invocation_id, node, status, started_at, finished_at,
...
WHERE share_token_hash = encode(sha256('<raw-token>')::bytea, 'hex');
```
...
2. Verify the catalog prompt prefix is being cached (look for `cache_read_input_tokens` in run_steps)
